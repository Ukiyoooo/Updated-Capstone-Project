...
if ($date_from!='' && $date_to !='') {
    $date_filter = "WHERE (`DueDate` BETWEEN '".$date_from."' AND '".$date_to."')";
}else if ($date_from!='' && $date_to =='') {
    $date_filter = "WHERE (`DueDate` LIKE '".$date_from."')";
}else if ($date_from=='' && $date_to !='') {
    $date_filter = "WHERE (`DueDate` BETWEEN '%%%%-%%-%%' AND '".$date_to."')";
}else{
    $date_filter="WHERE (`DueDate` LIKE '%')";
}
    $query = "SELECT `ID`, `AcademicYear`, `Semester`, `PatronID`, `PatronName`, `UserType`, `Section_Position`, `ItemID`, `ISBN`, `ItemCategory`,`Title_Name`, `Description`, `DateBorrowed`, `DueDate`, `DateReturned`, `Remarks` FROM `tbl_transactions` ".$date_filter;


$columns = array("AcademicYear","Semester","PatronID","PatronName","UserType","Section_Position","ItemID","ISBN","Title_Name","ItemCategory","Description","DateBorrowed","DueDate","DateReturned","ID");
...
if ($row["DateReturned"]=='0000-00-00 00:00:00'){
  $datenow = new DateTime($now);  
}else{
     $datenow = new DateTime($row["DateReturned"]);
}
...
$penalty = $days*5;	
 ...
 $sub_array[] = '<div id="td_name'.$row["ID"].'"><center>N/A</center></div>';
  ...
  $sub_array[] = '<div id="td_name'.$row["ID"].'"><center>'.$row["DateReturned"].'</center></div>';  
 }
...
require ("../../db_connection/myConn.php");
 $query = "SELECT `ID`, `AcademicYear`, `Semester`, `PatronID`, `PatronName`, `UserType`, `Section_Position`, `ItemID`, `ISBN`, `ItemCategory`,`Title_Name`, `Description`, `DateBorrowed`, `DueDate`, `DateReturned`, `Remarks` FROM `tbl_transactions`";
