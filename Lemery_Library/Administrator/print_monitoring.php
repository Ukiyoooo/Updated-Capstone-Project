 ...
 <div class="row">
  <div class="container">
      <center><h6><img src="../assets/img/logo2.png" style="width: 35px;">Lemery Colleges</h6></center>
            ...
            <center><h5>Patrons' Monitoring</h5></center>
            <?php
   ...
   <table class="table table-bordered" id="tbl_book_report">
     ...
     require ("../db_connection/myConn.php");
      ...
      $date_filter='';
      if ($date_from!='' && $date_to !='') {
        $date_filter = "WHERE (`DateBorrowed` BETWEEN '".$date_from."' AND '".$date_to."')";
      }else if ($date_from!='' && $date_to =='') {
        $date_filter = "WHERE (`DateBorrowed` LIKE '".$date_from."')";
      }else if ($date_from=='' && $date_to !='') {
        $date_filter = "WHERE (`DateBorrowed` BETWEEN '%%%%-%%-%%' AND '".$date_to."')";
      }else{
        $date_filter="WHERE (`DateBorrowed` LIKE '%')";
      }
...
$material = 'ID: '.$row["ItemID"].'<br>Title/Name: '. $row["Title_Name"]. '<br>Description: '.$row["Description"];
          $output.= '<tr>';  
            ...
            $output.='<td id="td_name'.$row["ID"].'"><center>'.$row["DueDate"].'</center></td>';
             $output.='<td id="td_name'.$row["ID"].'"><center>'.$row["DateReturned"].'</center></td>';
