 ...
 <div class="row">
  <div class="container">
      <center><h6><img src="../assets/img/logo2.png" style="width: 35px;">Lemery Colleges</h6></center>
            ...
            <center><h5>Book Inventory</h5></center>
            <?php
   ...
   <table class="table table-bordered" id="tbl_book_report">
     ...
     require ("../db_connection/myConn.php");
      ...
      if ($date_from!='' && $date_to !='') {
        ...
        $query = "SELECT  DISTINCT(`ISBN`) AS ISBN,`ID`, `Section`, `Title`, `Subject`, `Author`, `Publisher`, `Edition`, `Copyright`, MIN(ID) AS MINID, SUM(`Arrivals`) AS Arrivals, SUM(`Borrowed`) AS Borrowed, SUM(`Returned`) AS Returned, MAX(`ID`) AS MAXID FROM `tbl_inventorylogs_books`".$date_filter." GROUP BY ISBN";
...
$material = 'ISBN: '.$row["ISBN"].'<br>Subject: '. $row["Subject"]. '<br>Title: '.$row["Title"];
          $output.= '<tr>';  
